...
OP(movep_32_er)
{
	EA_DI(NA, Y)
	res  = READ_MEM_8(adr + 0); res <<= 8;
	res |= READ_MEM_8(adr + 2); res <<= 8;
	res |= READ_MEM_8(adr + 4); res <<= 8;
...
OP(movep_32_re)
{
	EA_READ_D(32, X, res)
	EA_DI(NA, Y)
	WRITE_MEM_8(adr + 6, res);	res >>= 8;
	WRITE_MEM_8(adr + 4, res);	res >>= 8;
	WRITE_MEM_8(adr + 2, res);	res >>= 8;
	WRITE_MEM_8(adr + 0, res);
	RET(24)
}
