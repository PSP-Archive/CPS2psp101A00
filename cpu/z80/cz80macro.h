...
#define SET_PC(A)				\
	/*CPU->*/ZZ->BasePC = (u32) /*CPU->*/ZZ->Fetch[(A) >> CZ80_FETCH_SFT];  \
	PC = (A) + /*CPU->*/ZZ->BasePC;

#define READ_BYTE(A, D)				 \
	D = /*CPU->*/ZZ->Read_Byte(A);

#if CZ80_LITTLE_ENDIAN
#define READ_WORD(A, D)				 \
	D = /*CPU->*/ZZ->Read_Byte(A) | (/*CPU->*/ZZ->Read_Byte((A) + 1) << 8);
#else
#define READ_WORD(A, D)				 \
...
#define WRITE_BYTE(A, D)	/*CPU->*/ZZ->Write_Byte(A, D);

#if CZ80_LITTLE_ENDIAN
#define WRITE_WORD(A, D)				\
	/*CPU->*/ZZ->Write_Byte(A, D);			  \
	/*CPU->*/ZZ->Write_Byte((A) + 1, (D) >> 8);
#else
#define WRITE_WORD(A, D)				\
	CPU->Write_Byte(A, D);			  \
...
#define IN(A, D)			/*D = 0xff*//*CPU->IN_Port(A)*/;
...
#define CHECK_INT													\
	...
	{																\
		...
		/*CPU->*/ZZ->HaltState = 0;											\
		zIFF1 = zIFF2 = 0;											\
		IntVect = /*CPU->*/ZZ->Interrupt_Ack(0);							\
																	\
		if (zIM == 2)												\
		{															\
			PUSH_16(zRealPC)										\
