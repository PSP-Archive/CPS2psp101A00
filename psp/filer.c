...
static int load_zipname(void)
{
	...
	sprintf(path, "%szipnamej.dat", launchDir);
	...
	if (!found)
	{
		sprintf(path, "%szipname.dat", launchDir);
	}
	if ((fp = fopen(path, "rb")) == NULL)
		return 0;
	...
	if ((zipname_buf = malloc(size + 2)) == NULL)
	{
		fclose(fp);
		return 0;
	}

	memset(zipname_buf, 0, size + 2);
	fread(zipname_buf, 1, size, fp);
	fclose(fp);

	zipname_num = 0;
	p = zipname_buf;

	while (zipname_num < 256)
	{
		zipname[zipname_num].flag = 0;
		zipname[zipname_num].zipname = p;
		while (*p)
		{
			if (*p == ',')
			{
		...
		zipname[zipname_num].title = p;
		while (*p)
		{
			if (*p == ',')
			{
				*p++ = '\0';
				if (!strnicmp(p, "GAME_NOT_WORK", 13))
...
void file_browser(void)
{
	...
	//uifont_print_center(136-20, 255,255,120, APPNAME_STR " " VERSION_STR "  by NJ(test)");
	...
	if (!load_zipname())
	{
		fatalerror("Could not open zipname.dat");
		show_fatal_error();
		show_exit_screen();
		goto error;
	}
