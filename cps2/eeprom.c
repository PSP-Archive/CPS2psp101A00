...
ADHOC_SEND( eeprom )
{
	...
	send_long(&serial_count, 1);
	send_long(&eeprom_data_bits, 1);
	send_long(&eeprom_read_address, 1);
	send_long(&eeprom_clock_count, 1);
	send_long(&latch, 1);
	send_long(&reset_line, 1);
	send_long(&clock_line, 1);
	send_long(&sending, 1);
	send_long(&locked, 1);
	send_long(&reset_delay, 1);
	send_byte(serial_buffer, SERIAL_BUFFER_LENGTH);
	send_byte(eeprom_data, MEMORY_SIZE);
}
...
ADHOC_RECV( eeprom )
{
	...
	recv_long(&serial_count, 1);
	recv_long(&eeprom_data_bits, 1);
	recv_long(&eeprom_read_address, 1);
	recv_long(&eeprom_clock_count, 1);
	recv_long(&latch, 1);
	recv_long(&reset_line, 1);
	recv_long(&clock_line, 1);
	recv_long(&sending, 1);
	recv_long(&locked, 1);
	recv_long(&reset_delay, 1);
	recv_byte(serial_buffer, SERIAL_BUFFER_LENGTH);
	recv_byte(eeprom_data, MEMORY_SIZE);
}
